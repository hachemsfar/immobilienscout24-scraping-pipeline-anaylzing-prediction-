...
# Import your custom modules properly
from your_module import Extract_Transform, Transform, Load
...
default_arguments = {
    'owner': 'Hachem SFAR',
    'email': ['hachem.sfar@supcom.tn'],  # Fix the email format
...
with DAG('Immobilienscout24 Pipeline', description='Immobilienscout24', schedule_interval="@daily", start_date=datetime(2022, 1, 17), catchup=False) as dag:
    ...
    ExtractTransform_task = PythonOperator(
        ...
        python_callable=Extract_Transform.Extract_and_Transform,  # Fix the callable
    )
    ExtractTransform_task.doc = dedent(
        """\
        ...
        The Transform task is to parse the JSON response and collect the features into a dictionary file
        """
    )

    load_task = PythonOperator(
        task_id='Load',
        python_callable=Load.load  # Fix the callable
    )
    load_task.doc = dedent(
        """\
        #### Load task
    ...
    success_task.doc = dedent(
        """\
        #### success task
        A simple success task which sends an email when the pipeline doesn't work fine
        """
    )
